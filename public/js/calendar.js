    ...
    const calendar = new FullCalendar.Calendar(document.getElementById('calendar'), {
        ...
        customButtons: {
            printButton: {
                text: 'Print',
                click: function() {
                    var printContents = document.getElementById("container-app").outerHTML;
        ...
        dateClick: function (info) {
            if(!containsEvent(info.dateStr)){
                ...
                document.getElementById('addDateRangePicker').value = info.dateStr
                document.getElementById('addDateRangePickerEnd').value = info.dateStr               
            }
        },
        select: function(info) {
            if(!containsEvent(info.dateStr)) {
                openModal('addShiftModal')
                document.getElementById('addDateRangePicker').value = info.startStr
                document.getElementById('addDateRangePickerEnd').value = info.endStr
            //     calendar.addEvent({
            //         start: info.startStr,
            //         end: info.endStr,
            //         allDay: true
